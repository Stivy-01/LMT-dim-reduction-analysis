...
class BehaviorProcessor:
    ...
    def process_events(self):
        ...
        with self.conn:
            ...
            self.conn.execute(f"CREATE TABLE behavior_hourly ({', '.join(behavior_columns)})")
            
            self.conn.execute("DROP TABLE IF EXISTS group_events_hourly")
            self.conn.execute(f"CREATE TABLE group_events_hourly ({', '.join(group_columns)})")
